# Descriptors\GradebookEntryTypeDescriptorsApi
...
| [**getGradebookEntryTypeDescriptorsPartitions()**](GradebookEntryTypeDescriptorsApi.md#getGradebookEntryTypeDescriptorsPartitions) | **GET** /ed-fi/gradebookEntryTypeDescriptors/partitions | Retrieves a set of page tokens to be used for efficient client-side parallel processing. |
...
```php
getGradebookEntryTypeDescriptors($offset, $limit, $page_token, $page_size, $min_change_version, $max_change_version, $total_count, $code_value, $description, $effective_begin_date, $effective_end_date, $id, $namespace, $short_description, $use_snapshot): \Descriptors\Model\EdFiGradebookEntryTypeDescriptor[]
```
...
try {
    $result = $apiInstance->getGradebookEntryTypeDescriptors($offset, $limit, $page_token, $page_size, $min_change_version, $max_change_version, $total_count, $code_value, $description, $effective_begin_date, $effective_end_date, $id, $namespace, $short_description, $use_snapshot);
    print_r($result);
} catch (Exception $e) {
...
```php
getGradebookEntryTypeDescriptorsPartitions($number, $min_change_version, $max_change_version, $code_value, $description, $effective_begin_date, $effective_end_date, $id, $namespace, $short_description, $use_snapshot): \Descriptors\Model\GetAbsenceEventCategoryDescriptorsPartitions200Response
```
...
try {
    $result = $apiInstance->getGradebookEntryTypeDescriptorsPartitions($number, $min_change_version, $max_change_version, $code_value, $description, $effective_begin_date, $effective_end_date, $id, $namespace, $short_description, $use_snapshot);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling GradebookEntryTypeDescriptorsApi->getGradebookEntryTypeDescriptorsPartitions: ', $e->getMessage(), PHP_EOL;
}
```
